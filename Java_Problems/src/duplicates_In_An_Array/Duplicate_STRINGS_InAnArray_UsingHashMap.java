...
public class Duplicate_STRINGS_InAnArray_UsingHashMap {
	...
	public static void main(String[] args) { //Entry point of the Java program.
		...
		//This map will store each character along with its count of occurrences.
		//Creates a new empty HashMap newobj.
		 // keys - unique
		// value - duplicates
		
		int count =1; //Initializes an integer variable count to 1. This will be used to count occurrences of each character.
		...
		for(int i=0; i<a.length; i++)//Loops through each character in the array a.
		{
			...
			{
		    newobj.put(a[i], count);//If newobj does not contain currentChar, it adds the character a[i] as a key with an initial count (count) of 1.
		    //Prints a message indicating that the character currentChar is added to the HashMap with an initial count of 1.
			}else
			{
				newobj.put(a[i],newobj.get(a[i])+1);//If yes (else block), it increments the count of occurrences associated with a[i] by 1.
				/*
				 * If newobj contains currentChar:
			int currentCount = newobj.get(currentChar);: Retrieves the current count of currentChar from the HashMap.
			newobj.put(currentChar, currentCount + 1);: Updates the count of currentChar in the HashMap by incrementing its current count by 1.
			System.out.println("Incremented count of '" + currentChar + "' to " + (currentCount + 1));: 
					Prints a message indicating that the count of currentChar has been incremented.*/
			}
		...
		for(Character i:newobj.keySet())//After populating the HashMap, another loop iterates through each key (Character i) in the HashMap.
		{
			...
			// Prints each character (i) along with its count of occurrences (newobj.get(i)).
		}
