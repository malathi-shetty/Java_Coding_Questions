...
public class Find_Common_Elements_In_An_Array_1 {
...
printing the HashSet at the moment when a duplicate is found, 
showing the set’s unique contents up to that point.


int[] a = { 1, 2, 3, 3, 4, 5, 5, 6, 7,7,7,8 };
The println(newobj) statement is only executed for elements that are duplicates. 
Since 8 is not a duplicate when it is encountered (it has not been seen before), 
the code inside the if block does not run, and thus 8 does not trigger a print.
