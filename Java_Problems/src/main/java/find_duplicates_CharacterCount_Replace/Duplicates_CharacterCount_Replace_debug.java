...
public class Duplicates_CharacterCount_Replace_debug {
	...
	public static String CharacterCountReplace(String input) {
        ...
        System.out.println("Initialized character count map.");

        // Convert the input string to a char array
        char[] chars = input.toCharArray();
        System.out.println("Converted input string to char array.");

        // Count the frequency of each character (case-insensitive)
        for (char c : chars) {
            char lowerCaseChar = Character.toLowerCase(c);
            int currentCount = charCountMap.getOrDefault(lowerCaseChar, 0) + 1;
            charCountMap.put(lowerCaseChar, currentCount);
            System.out.println("Processed character: '" + c + "' (lowercase: '" + lowerCaseChar + "'). Current count: " + currentCount);
        }
        ...
        for (char c : chars) {
            ...
            if (count > 1) {
                result.append(count);
                System.out.println("Character '" + c + "' is a duplicate. Appended count: " + count);
            } else {
                result.append(c);
                System.out.println("Character '" + c + "' is unique. Appended character.");
            }
        }
